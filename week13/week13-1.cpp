///week13-1.cpp 聖誕倒數寫程式Advent of Code 2025第一天第一題
...
///在Adentofcode.com 登人你的 GitHub帳能後,便可看到你的專屬迷題Input
///貼到右下角 stdin 後,再執行,看你的答案是多少(每個人都不一楼)
int main() {
    char c;///字母, 應方向 L左轉 R右轉
    int d;///數字,要轉動幾格
    int now=50;/// 一開始的密碼鎖,指向50
    int ans=0;///轉動時,有幾次停在0的地方?(我們的通關密碼)
    while ( cin >> c >> d){///一直讀資料:讀資料，讀字母
        ///if (c=='L') cout << "往左轉" << d <<"格\n";
        ///if (c=='R') cout << "往右轉" << d <<"格\n";
        ///打上面兩行的中文訊息註解掉
        if (c=='L') now = now - d ; ///减掉
